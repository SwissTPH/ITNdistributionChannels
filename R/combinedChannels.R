...
generate_ContAndContinuousDistr_SpecificAges=function(halflife_weibull=2.1,
  ...
  if(overlap){
    ...
    combined=merge(cont1,cont2) |>
      dplyr::mutate(cov_targetgroup1=totcov_targetgroup1+totcov_othergroup2,
                    cov_targetgroup2=totcov_targetgroup2+totcov_othergroup1,
                    spillover2_cov_othergroup1=pmax(0, cov_targetgroup1-1)*ageGroupProp_cont1/(1-ageGroupProp_cont1),
                    spillover2_cov_othergroup2=pmax(0, cov_targetgroup2-1)*ageGroupProp_cont2/(1-ageGroupProp_cont2),
                    cov_othergroup=totcov_othergroup1+totcov_othergroup2+spillover2_cov_othergroup1+spillover2_cov_othergroup2,
                    cov_targetgroup1=cov_targetgroup1+spillover2_cov_othergroup2,
                    cov_targetgroup2=cov_targetgroup2+spillover2_cov_othergroup1,
...
generate_AlternateMassCont_uniformAllAges=function(halflife_weibull=2.1,
  ...
  cont=generate_massCampaign_UniformAllAges(halflife_weibull=halflife_weibull,
                                               ...
                                               reach=reach_cont, max_usage=max_usage,
                                               ageGroupProp=ageGroupProp_cont) |>
    dplyr::select(-use_total, -reach)

  cont_tot = cont |> dplyr::rename(cov_cont=cov) |> mutate(time=time+365)
  for(i in 2:(frequency_mass-1)){
    cont_tot=merge(cont_tot,
                   cont |> dplyr::rename(cov_contY2=cov) |> mutate(time=time+i*365),
                   all=T) |>
      mutate(cov_cont=ifelse(is.na(cov_cont),0,cov_cont)+ifelse(is.na(cov_contY2),0,cov_contY2)) |>
      select(time, halflife, cov_cont)
  }
  ...
  combined=merge(mass |> dplyr::select(-use_total) |>  dplyr::rename(cov_mass=cov, reach_mass=reach),
    ...
    dplyr::mutate(cov=cov_mass+ifelse(is.na(cov_cont),0,cov_cont),
                  use_total=pmin(cov*max_usage, max_usage))|>
    dplyr::select(-cov_cont, -cov_mass)|>
    dplyr::mutate(reach_cont=reach_cont)
...
generate_AlternateMassCont_SpecificAges=function(halflife_weibull=2.1,
  ...
  cont_tot = cont  |> mutate(time=time+365)
  for(i in 2:(frequency_mass-1)){
    cont_tot=merge(cont_tot,
                   cont |> dplyr::rename(cov_target_contY2=totcov_targetgroup, cov_other_contY2=totcov_othergroup) |> mutate(time=time+i*365),
                   all=T) |>
      mutate(totcov_targetgroup=ifelse(is.na(totcov_targetgroup),0,totcov_targetgroup)+ifelse(is.na(cov_target_contY2),0,cov_target_contY2),
             totcov_othergroup=ifelse(is.na(totcov_othergroup),0,totcov_othergroup)+ifelse(is.na(cov_other_contY2),0,cov_other_contY2)) |>
      select(time, totcov_targetgroup, totcov_othergroup)
  }
...
generate_AlternateMassContRoutine_uniformAllAges=function(halflife_weibull=2.1,
  ...
  combined=merge(mass |> dplyr::select(-use_total) |>  dplyr::rename(cov_mass=cov, reach_mass=reach),
    ...
    dplyr::select(-cov_cont, -cov_mass, -cov_routine)
...
generate_AlternateMassContRoutine_SpecificAges=function(halflife_weibull=2.1,
  ...
  combined=merge(merge(mass |> dplyr::select(-use_total) |>  dplyr::rename(cov_mass=cov, reach_mass=reach),
                 cont_tot |> dplyr::mutate(reach_cont=reach_cont), all.x = T),
    ...
    dplyr::mutate(totcov_targetgroup=ifelse(is.na(totcov_targetgroup),0,totcov_targetgroup),
                  ...
                  spillover2_cov_othergroup=pmax(0, cov_targetgroup-1)*ageGroupProp_cont/(1-ageGroupProp_cont),
                  spillover2_cov_othergroup_routine=pmax(0, cov_targetgroup_routine-1)*ageGroupProp_routine/(1-ageGroupProp_routine),
                  ...
                  cov_targetgroup=cov_targetgroup+spillover2_cov_othergroup_routine,
                  cov_targetgroup_routine=cov_targetgroup_routine+spillover2_cov_othergroup,
                  use_targetpop=pmin(cov_targetgroup*max_usage, max_usage),
